...
uint32_t circular_buffer_dequeue(circular_buffer_t * p_this, uint8_t * p_buffer, uint32_t size)
{
    ...
    if (p_this->head_index < p_this->tail_index)
    {
        ...
        memcpy(&p_this->p_buffer[p_this->head_index], &p_buffer[0], result);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                memcpy(&p_this->p_buffer[0], &p_buffer[result], size - result);
                p_this->head_index = size - result;
                result = size;
            }
            else
            {
                memcpy(&p_this->p_buffer[0], &p_buffer[result], p_this->tail_index);
